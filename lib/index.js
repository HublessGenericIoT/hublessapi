...
var deleteDeviceById = require('../lib/handlers/DeleteDeviceById');
...
router.delete("/devices/:id", function(req, res, next) {
    deleteDeviceById.handler(req.params.id, req.body, function basicResponder(err, result) {
        if(err) {
            console.log(err);
            if(err.status && err.status === "Error" && err.errorMessage) //if it is an error we planned for.
                res.status(400).send(err);
...
require("./aws").init(function(err, result) {
